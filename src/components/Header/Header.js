...
import ToggleMenu from "./ToggeleMenu";
...
function Header({ history }) {
  const { isMenuOpen, isMobile, closeMenu, openMenu, pageTitle, ScrollYValue, contact, loading,setInit, initial, vid,setTransition } = useAppContext();
  ...
  useEffect(()=>{
    ...
    if (contact === "Contact"){
      ...
      gsap.to(q(".estimateBtn"), {
        autoAlpha:()=> loading ? 0 :1,
        duration:.3,
        delay:()=>initial ?.8:0,
      })
    }

  },[contact, loading, document.title])
  useEffect(() => {
    if( ScrollYValue === 0 ) {
  ...
  useEffect(()=>{
    // if(ScrollYValue === 10){
      ...
      gsap.to(q(".logo, .menu"),{
        ...
        duration:.35,
        delay:1.251,
        onComplete:()=>setInit("initial"),
      })
      gsap.to(navlinks, {
        y: () => (ScrollYValue === 10 ? -20 : 0),
        duration: 0.3,
        delay:()=>initial?1.25:0,
    ...
    // }
    else if (loading && document.title === "Design Studio | AM-ARC | Architecture & Interior"){
      // const q = gsap.utils.selector(el);
      
      const elems = q(".fadelinks, .logo, .estimateBtn, .menu ");
      gsap.set(elems, {
        autoAlpha:0,
      })

    }
  }, [ScrollYValue, loading,document.title, vid])
  // useEffect(()=>{
  //   const q = gsap.utils.selector(el);
  //   const elems = q(".fadelinks, .logo, .estimateBtn, .menu ");
  //   gsap.set(elems, {
  ...
  const closeMenuIfOpen = (url) => {
    if (isMenuOpen && menuBg) {
      menuBg.current.click();
    }
    if ( url === "/" && url !== lastLocation ) {
    
      // it will navigawait wait(500);
      setTimeout(() => {
        // console.log("wait")
